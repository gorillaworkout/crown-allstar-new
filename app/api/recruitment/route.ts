...
export async function POST(request: Request) {
  ...
  if (state === 'before') {
    return bad('Pendaftaran Angkatan 18 belum dibuka. Buka 13 Agustus 2026.', 403);
  }
  if (state === 'closed') {
    return bad('Pendaftaran Angkatan 18 sudah ditutup pada 21 Agustus 2026.', 403);
  }
  ...
  try {
    ...
    return bad('Format data tidak valid.');
  }
  ...
  if (fullName.length < 3) return bad('Nama lengkap minimal 3 karakter.');

  const birthDate = str(body.birthDate, 10);
  const age = ageAt(birthDate);
  if (age < 0) return bad('Tanggal lahir tidak valid.');
  if (age < 13) return bad('Minimum usia pendaftaran adalah 13 tahun.');
  if (age > 60) return bad('Tanggal lahir tidak valid.');

  const gender = str(body.gender, 20) as Gender;
  if (gender !== 'perempuan' && gender !== 'laki-laki') return bad('Pilih jenis kelamin.');

  const division = str(body.division, 20) as Division;
  if (!DIVISIONS.some((d) => d.id === division)) return bad('Pilih divisi.');
  ...
  if (!whatsapp) return bad('Nomor WhatsApp tidak valid. Contoh: 081234567890');

  const domicileCity = str(body.domicileCity, 60);
  if (!domicileCity) return bad('Domisili wajib diisi.');

  const previousTeam = str(body.previousTeam, 80);
  const isBeginner = body.isBeginner === true;
  if (!isBeginner && !previousTeam) {
    return bad('Isi tim cheers sebelumnya, atau centang "belum pernah".');
  }
  ...
  if (position.length === 0) return bad('Pilih minimal satu posisi.');

  const emergencyName = str(body.emergencyName, 80);
  const emergencyPhone = normalizeWhatsapp(str(body.emergencyPhone, 25));
  if (!emergencyName) return bad('Nama kontak darurat wajib diisi.');
  if (!emergencyPhone) return bad('Nomor kontak darurat tidak valid.');

  // Both are hard requirements from Bayu — not optional checkboxes.
  if (body.parentApproval !== true) return bad('Persetujuan orang tua wajib dicentang.');
  if (body.commitmentAgree !== true) return bad('Kesediaan komitmen wajib dicentang.');
  ...
  try {
    ...
    if (!dupe.empty) {
      return bad('Nomor WhatsApp ini sudah terdaftar untuk Angkatan 18.', 409);
    }
    ...
    if (ip !== 'unknown') {
      ...
      if (recent.size >= MAX_PER_IP_PER_HOUR) {
        return bad('Terlalu banyak pendaftaran dari jaringan ini. Coba lagi nanti.', 429);
      }
    }
    ...
    return bad('Gagal menyimpan pendaftaran. Coba lagi atau hubungi kami via WhatsApp.', 500);
  }
}
